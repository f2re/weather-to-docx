...
Если `WTD_DADATA_TOKEN` не задан, приложение автоматически использует бесплатный
OpenStreetMap Nominatim для поиска городов, адресов и обратного геокодирования.
Для интенсивной или закрытой установки укажите собственный совместимый endpoint:

```dotenv
WTD_NOMINATIM_URL="https://nominatim.openstreetmap.org"
```

DaData остаётся приоритетным провайдером, если задан его token; его `secret`
добавляет пакетную стандартизацию адресов.
