...
def _location_payload(identifier: str, name: str, latitude: float, longitude: float) -> dict:
    return {
        "id": identifier,
        "name": name,
        "latitude": latitude,
...
def test_persistent_operator_workflow_after_restarts(tmp_path: Path) -> None:
    ...
    # Первый запуск API: оператор наполняет постоянный справочник.
    ...
    # Второй запуск API: справочник сохранён, из него создаётся пакетная задача.
    with TestClient(create_app(settings)) as client:
        stored_locations = client.get("/api/v1/locations?limit=100").json()
        assert [item["id"] for item in stored_locations] == ["point-one", "point-two"]
    ...
    # API не запущен: отдельный worker восстанавливает задачу из SQLite и обрабатывает её.
    assert run_worker(settings, once=True) == 1

    # Третий запуск API: результат и артефакты доступны после перезапуска.
    with TestClient(create_app(settings)) as client:
        ...
        zip_response = client.get(f"/api/v1/jobs/{job_id}/artifacts/{zip_index}")
