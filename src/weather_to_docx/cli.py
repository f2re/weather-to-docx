...
from weather_to_docx.domain.models import BatchRequest, DocumentOptions, Location, SourceRequest
...
def generate(
    config: Annotated[Path, typer.Option("--config", "-c", exists=True, readable=True, help="YAML с координатами и источниками")],
    output: Annotated[Path | None, typer.Option("--output", "-o", help="Каталог результатов")] = None,
) -> None:
    """Сразу получить прогнозы и сформировать отдельный DOCX для каждой координаты."""
    settings = _settings()
    request = _load_batch_request(config)
    result = asyncio.run(ForecastBatchService(settings).generate(request, output_root=output))
...
@app.command("worker")
def worker(
    once: Annotated[bool, typer.Option("--once", help="Обработать не более одного задания и завершиться")] = False,
    poll_interval: Annotated[float, typer.Option("--poll-interval", min=0.2, max=300)] = 5.0,
) -> None:
...
@app.command("sample")
def sample(
    output: Annotated[Path, typer.Option("--output", "-o", help="Каталог примера")] = Path("var/sample"),
    hours: Annotated[int, typer.Option("--hours", min=6, max=168)] = 48,
) -> None:
    ...
    request = BatchRequest(
        ...
        sources=[SourceRequest(source_id="demo", forecast_days=max(1, (hours + 23) // 24), options={"hours": hours})],
        document=DocumentOptions(title="Демонстрационный метеорологический прогноз"),
...
def collect_bundle(
    config: Annotated[Path, typer.Option("--config", "-c", exists=True, readable=True)],
    output: Annotated[Path, typer.Option("--output", "-o", help="Файл .tar.zst")],
    private_key: Annotated[Path | None, typer.Option("--private-key", exists=True, readable=True)] = None,
) -> None:
    ...
    errors = [f"{item.location.name}: {error}" for item in collected for error in item.errors]
    if not series:
...
def generate_bundle(
    bundle: Annotated[Path, typer.Option("--bundle", "-b", exists=True, readable=True)],
    output: Annotated[Path | None, typer.Option("--output", "-o")] = None,
    public_key: Annotated[Path | None, typer.Option("--public-key", exists=True, readable=True)] = None,
    require_signature: Annotated[bool, typer.Option("--require-signature")] = False,
    page_size: Annotated[str, typer.Option("--page-size", case_sensitive=False)] = "A3",
) -> None:
    ...
    content = ForecastBundle.read(
        ...
        require_signature=require_signature or settings.require_bundle_signature,
    )
...
def generate_keys(
    private_key: Annotated[Path, typer.Option("--private-key", help="Закрытый ключ PEM")] = Path("keys/forecast-bundle-private.pem"),
    public_key: Annotated[Path, typer.Option("--public-key", help="Открытый ключ PEM")] = Path("keys/forecast-bundle-public.pem"),
) -> None:
    """Создать пару Ed25519 для подписи переносимых прогнозных пакетов."""
    if private_key.exists() or public_key.exists():
        raise typer.BadParameter("Файл ключа уже существует; автоматическая перезапись запрещена")
...
@app.command("doctor")
def doctor(
    deep: Annotated[bool, typer.Option("--deep", help="Дополнительно сформировать короткий DOCX")] = False,
) -> None:
    """Проверить каталоги, SQLite, zstd, ecCodes и генератор Word."""
    settings = _settings()
    JobRepository(settings.database_path).initialise()
    checks = {
        "python": True,
    ...
    if deep:
        with tempfile.TemporaryDirectory(prefix="weather-to-docx-doctor-") as temporary:
            request = BatchRequest(
                locations=[Location(id="doctor", name="Проверка", latitude=0, longitude=0, timezone="UTC")],
                sources=[SourceRequest(source_id="demo", forecast_days=1, options={"hours": 6})],
            )
            result = asyncio.run(
                ForecastBatchService(settings).generate(request, output_root=Path(temporary))
            )
            checks["docx_generation"] = bool(result.artifacts) and result.status.value != "failed"
    typer.echo(json.dumps(checks, ensure_ascii=False, indent=2))
    required = ["python", "data_dir", "database", "documents_writable", "zstd"]
