    ...
    async def fetch(
        ...
        for lead in range(hours + 1):
            ...
            values = {
                ...
                "apparent_temperature": ForecastValue(value=temperature - wind_speed * 0.25, unit="°C", quality=QualityFlag.CALCULATED),
                "dew_point_2m": ForecastValue(value=temperature - 4.5, unit="°C"),
                "relative_humidity_2m": ForecastValue(value=max(35, min(98, 78 - solar_phase * 22)), unit="%"),
                "pressure_msl": ForecastValue(value=1012 + 5 * math.sin(lead / 18), unit="гПа"),
                "surface_pressure": ForecastValue(value=1003 + 5 * math.sin(lead / 18), unit="гПа"),
                "wind_speed_10m": ForecastValue(value=wind_speed, unit="м/с"),
                "wind_direction_10m": ForecastValue(value=(210 + lead * 7) % 360, unit="°"),
                "wind_gusts_10m": ForecastValue(value=wind_speed * 1.65, unit="м/с"),
                "precipitation": ForecastValue(value=precipitation, unit="мм"),
                "rain": ForecastValue(value=precipitation, unit="мм"),
                "precipitation_probability": ForecastValue(value=75 if precipitation else 10, unit="%"),
                ...
                "visibility": ForecastValue(value=15_000 if precipitation else 30_000, unit="м"),
                "cape": ForecastValue(value=900 if precipitation else 80, unit="Дж/кг"),
                "cin": ForecastValue(value=-25 if precipitation else -90, unit="Дж/кг"),
                "shortwave_radiation": ForecastValue(value=max(0, solar_phase) * 620, unit="Вт/м²"),
                "direct_radiation": ForecastValue(value=max(0, solar_phase) * 410, unit="Вт/м²"),
                "diffuse_radiation": ForecastValue(value=max(0, solar_phase) * 210, unit="Вт/м²"),
                "sunshine_duration": ForecastValue(value=3600 if solar_phase > 0.35 and cloud < 65 else 0, unit="с"),
                "vapour_pressure_deficit": ForecastValue(value=max(0, 1.5 + solar_phase), unit="кПа"),
                "et0_fao_evapotranspiration": ForecastValue(value=max(0, solar_phase) * 0.14, unit="мм"),
                "soil_temperature_0cm": ForecastValue(value=temperature - 1, unit="°C"),
                "soil_temperature_6cm": ForecastValue(value=12.5 + 3 * solar_phase, unit="°C"),
        ...
        return ForecastSeries(
            ...
            points=points,
            warnings=["Демонстрационный ряд не является реальным прогнозом и не должен использоваться в работе."],
        )
