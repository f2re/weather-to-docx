...
class JobRepository:
    ...
    def heartbeat(
        ...
        *,
        ...
        """Продлить аренду только у активного задания данного worker."""
    ...
    def _update(
        ...
        *,
        ...
        now = datetime.now(UTC).isoformat()
        worker_id = None if clear_lease else self.get(job_id).worker_id
        lease = None if clear_lease else self.get(job_id).lease_expires_at_utc
        lease_text = lease.isoformat() if lease else None
        with self._connect() as connection:
            cursor = connection.execute(
                """
                UPDATE jobs
    ...
    def _recover_stale_jobs(
        ...
        cursor = connection.execute(
            ...
            SET status = ?, worker_id = NULL, lease_expires_at_utc = NULL,
                progress_message = ?, updated_at_utc = ?,
                error = ?
            WHERE status = ? AND lease_expires_at_utc IS NOT NULL
              AND lease_expires_at_utc < ?
            """,
